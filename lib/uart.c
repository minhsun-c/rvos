...
void uart_init()
{
     ...
     * receiver/transmitter and the interrupt enable register. To change what
     * the base address points to, we open the "divisor latch" by writing 1 into
     * the Divisor Latch Access Bit (DLAB), which is bit index 7 of the Line
     ...
     * is 3. And due to the divisor register is two bytes (16 bits), so we need
     * to split the value of 3(0x0003) into two bytes, DLL stores the low byte,
